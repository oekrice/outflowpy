...
def compare_angles(year):
    ...
    # ax.plot(np.linspace(0.0,1.0), label = "PFSS, $r_{ss} = 5.0R_\odot$", c = colors[3], linestyle = 'dashed')
    # ax.plot(np.linspace(0.0,1.0), label = "PFSS, $r_{ss} = 2.5R_\odot$", c = colors[3])
    # ax.plot(np.linspace(0.0,1.0), label = "Outflow, $r_{ss} = 5.0R_\odot$", c = colors[0], linestyle = 'dashed')
    # ax.plot(np.linspace(0.0,1.0), label = "Outflow, $r_{ss} = 2.5R_\odot$", c = colors[0])
...
for yi, year in enumerate(years):
    ax = axs[yi]
    compare_angles(year)   #Line plots happen in here
    ax.set_title(year)
    if yi//2 == 0:
        ax.set_xticks([])
    if yi%2 == 1:
...
xlabel_ax.axis("off")
xlabel_ax.text(0.5, 0.5, 'Altitude r ($R_\odot$)', ha="center", va="center")
