...
def make_plots(batch_set, copy_data = False):
    ...
    #Read in the data
        with LockedLog("./batch_logs/0_%d.txt" % batch_id, mode = "r+") as f:
            ...
            if len(ofluxes) > 5:
                ...
                all_ofluxes.append(interp_fn(time_interps))

        r = np.corrcoef(oflux_ref, all_ofluxes[-1])[0,1]
        m, c = np.polyfit(oflux_ref, all_ofluxes[-1], 1)

        xs = np.linspace(0, max(oflux_ref),10)
        plt.plot(xs, m*xs + c, color = colors[plot_num], label = (f'mf = {mf_constants[plot_num]}, r = {r:.3f}'))
        print('Correlation', r, 'Gradient', m, 'Avg fraction', np.mean(all_ofluxes[-1]/oflux_ref), 'npoints = ', len(ofluxes))

        plt.scatter(oflux_ref, all_ofluxes[-1], color = colors[plot_num], s = 0.25)
    ...
    plt.legend()
    plt.xlabel('Measure Flux at 1AU (Mx)')
    plt.ylabel('Flux Predicated by the Outflow Model')
    plt.ylim(ymin = 0.0, ymax = 0.2e24)
    plt.title(f'Corona temperature = {corona_temps[batch_set]/1000000} MK')

    plt.savefig('./plots/0_openflux_correlate_%d.png' % batch_set)
    plt.close()
