...
def find_obs_time(eclipse_year):
    #Find the eclipse date corresponding to the specified year
    with open('./data/eclipse_times.txt', mode = "r") as time_data:
...
for plot_count, eclipse_year in enumerate(years):
    ...
    for tracer_option in range(1):
        ...
        if True:
            ...
            ax.set_axis_off()
            plt.savefig(f'./plots/eclipse_{eclipse_year}_{plot_count:03d}.png')
            plt.close()


        else:
            outflowpy.plotting.plot_pyvista(outflow_out, field_lines, fname = f'./plots/vista_{eclipse_year}.png')
