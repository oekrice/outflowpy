...
def find_real_angle_distribution(eclipse_year, doplots = True):
    ...
    if doplots:
        ...
        ax.pcolormesh(R, T, bin_means.T, vmin = 0, vmax = np.pi/3, cmap = cmap)
        ax.set_axis_off()
        ax.axis('equal')
        ax.set_title(f'{eclipse_year}')

years = [2006,2008,2009,2010,2012,2013,2015,2016,2017,2019,2023,2024]

years = [2010, 2013, 2017, 2024]
nrows = 2
ncols = 2

fig, axs = plt.subplots(nrows,ncols, figsize = (6.9,6.9*nrows/ncols))


for yi, year in enumerate(years):
    ax = axs[yi//ncols, yi%ncols]
    find_real_angle_distribution(year)
plt.tight_layout()
plt.savefig('8_histograms.pdf')
plt.show()
plt.close()
