...
def make_plot():
    ...
    fig, axs = plt.subplots(2,1, figsize = (6.9,8.0))
    ...
    titles = ["$r_{ss} = 2.5R_\odot$", "$r_{ss} = 5.0R_\odot$"]
        ...
        #Read in the data
            with LockedLog("./data/batch_logs_mdi/0_%d.txt" % batch_id, mode = "r+") as f:
                ...
                for run, info in enumerate(f, start=0):
                    ...
                    if toYearFraction(date) < 1999.25 or toYearFraction(date) > 2022:
                        continue
            ...
            if batch_id == 0 or batch_id == 60:
                label = 'PFSS'
            else:
                label = "Optimised outflow field"

            oflux_ref = frost_interp(dates)

            r = np.corrcoef(oflux_ref, ofluxes)[0,1]

            print('Correlation for', label, 'r = ', r)
            print('Fraction', 1.0/np.mean(oflux_ref/ofluxes))
            #plt.plot(dates, ofluxes, color = colors[plot_num], linewidth = 0.1, linestyle = 'dashed')
            ax.plot(dates_mean, oflux_mean, color = colors[plot_num], label = label, linewidth = 1.0)
            ax.plot(dates_mean, oflux_min, color = colors[plot_num], linewidth = 0.5)
            ax.plot(dates_mean, oflux_max, color = colors[plot_num], linewidth = 0.5)

        #Plot open flux measurements
        ax.plot(tflux, oflux, c = 'black', linewidth = 1.0, label = 'In-situ measurement')
        ax.plot(tflux, ofluxmin, c = 'black', linewidth = 0.25)
        ax.plot(tflux, ofluxmax, c = 'black', linewidth = 0.25)

        ax.set_xticks(ticks = [2000,2005,2010,2015,2020])
        ax.set_title(titles[ax_id])
    #Plot optimised eclipse ofluxes
    ...
    #plt.scatter(times, eclipse_ofluxes, c = 'red', marker = 'x', label = 'Individually optimised eclipse', zorder = 10)
        ...
        if potentialcount > 0:
            ...
            ax.legend(ncol = 2)
            ax.set_xlabel('Year')
            ax.set_ylabel('Open Flux at 1AU (Mx)')
            ax.set_xlim(2000,2022)
            ax.set_ylim(ymin = 0.0, ymax = 0.16e24)
    plt.tight_layout()
    plt.savefig('./13_plot_optimums.pdf')
    plt.show()
    plt.close()
